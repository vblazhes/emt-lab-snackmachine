package mk.ukim.finki.emt.labs.dormitorysmartsnackmachine.sharedkernel.port.client;

import mk.ukim.finki.emt.labs.dormitorysmartsnackmachine.sharedkernel.infra.eventlog.RemoteEventLog;
 ...
 * {EventLogController}. Clients should create as many instances of this
...
//            return String.format("%s[self=%s, previous=%s, next=%s]", RemoteEventLog.class.getSimpleName(), thisUri, previousLogUri, nextLogUri);
            return String.format("%s[self=%s, previous=%s, next=%s]", RemoteEventLog.class.getSimpleName());
